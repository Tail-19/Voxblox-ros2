...
class SemSeg(Node):
    def __init__(self, cfg):
        ...
        self.subscription = self.create_subscription(Image, "/camera/image_raw", self.segformer_callback, 10)
    ...
    def preprocess(self, image: Tensor) -> Tensor:
        ...
        image = self.tf_pipeline(image).to(self.device) #tf_pipeline is a list of transformations
        return image
    ...
    def segformer_callback(self, rgb_msg):
        try:
            ...
            rgb_img = torch.tensor(rgb_img).to(self.device).permute(2,0,1) #permute the image to (C,H,W)
        ...
        with console.status("[bright_green]Processing..."):
            ...
            console.print('Single image predict time: '+ str(time.time() - start))
...
def main(args=None):
    ...
    parser.add_argument('--cfg', type=str, default='/home/alpha/Research/voxblox/src/segformer/segformer/semantic_segmentation/configs/ade20k.yaml')
